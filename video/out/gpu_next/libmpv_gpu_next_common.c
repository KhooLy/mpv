...
static void update_lut(struct gpu_next_priv *p, struct user_lut *lut);

static bool format_supported(struct gpu_next_priv *p, int imgfmt, bool use_uint)
{
...
static bool hwdec_acquire(pl_gpu gpu, struct pl_frame *frame)
{
    ...
    struct gpu_next_priv *p = mpi->priv;
    ...
    for (int n = 0; n < frame->num_planes; n++) {
        if (!(frame->planes[n].texture = p->wrap_hwdec_tex(p, p->hwdec_mapper->tex[n])))
            return false;
    }
    return true;
}
...
static void hwdec_release(pl_gpu gpu, struct pl_frame *frame)
{
    struct mp_image *mpi = frame->user_data;
    struct gpu_next_priv *p = mpi->priv;
    if (p->release_hwdec_planes)
        p->release_hwdec_planes(p, frame);
...
static void update_overlays(struct gpu_next_priv *p, struct mp_osd_res res,
                            struct mp_image *src, int osd_flags,
                            struct pl_frame *frame)
{
    if (!p->osd)
        return;
    ...
    for (int n = 0; n < subs->num_items; n++) {
        ...
        if (!item->num_parts || !item->packed)
            continue;
        struct lgn_osd_entry *entry = &p->overlays.entries[item->render_index];
...
static const struct pl_hook *load_hook(struct gpu_next_priv *p, const char *path)
{
    if (!path || !path[0])
        return NULL;
    ...
    MP_TARRAY_APPEND(p, p->user_hooks, p->num_user_hooks, (struct lgn_user_hook){
...
static void update_hook_opts_dynamic(struct gpu_next_priv *p, const struct pl_hook *hook,
...
static void update_hook_opts(struct gpu_next_priv *p, char **opts, const char *shaderpath,
...
static void apply_target_contrast(struct gpu_next_priv *p, struct pl_color_space *color)
{
...
static void apply_target_color(struct gpu_next_priv *p, struct pl_frame *target)
{
    const struct gl_video_opts *opts = p->opts_cache->opts;
...
static const struct pl_filter_config *map_scaler(struct gpu_next_priv *p,
                                                 enum scaler_unit unit)
{
    ...
    struct lgn_scaler_params *par = &p->scalers[unit];
                    ...
                    struct voctrl_screenshot *args)
{
    ...
    struct pl_queue_params qparams = *pl_queue_params(.pts = p->last_pts, .drift_compensation = 0);
    if (pl_queue_update(p->queue, &mix, &qparams) == PL_QUEUE_ERR) {
    ...
    if (args->res) {
        ...
        if (!pl_tex_download(gpu, pl_tex_transfer_params(
                .tex = fbo, .ptr = args->res->planes[0], .row_pitch = args->res->stride[0])))
        {
            TA_FREEP(&args->res);
        }
    }
