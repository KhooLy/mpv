...
static pl_tex wrap_hwdec_tex(struct priv *p, struct ra_tex *ratex)
{
...
static bool hwdec_acquire(pl_gpu gpu, struct pl_frame *frame)
{
    ...
    for (int n = 0; n < frame->num_planes; n++) {
        if (!(frame->planes[n].texture = wrap_hwdec_tex(p, p->hwdec_mapper->tex[n])))
            return false;
    }
    return true;
}
                  ...
                  struct vo_frame *frame)
{
    ...
    apply_crop(&target, p->dst, fbo->w, fbo->h);
