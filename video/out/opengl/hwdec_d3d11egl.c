...
static int init(struct ra_hwdec *hw)
{
    ...
    // Modern ANGLE advertises EGL_EXT_device_query as a client extension
    // rather than a display extension, so accept it from either string. The
    // eglQueryDisplayAttribEXT/eglQueryDeviceAttribEXT calls below still gate
    // on the functionality actually working.
